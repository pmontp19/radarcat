...
/// pública, mai `RadarCompositor`/`RainDetector`/`LocationProvider`
/// directament - l'única excepció és `location.authorizationDenied`, que els
/// Ajustos necessiten per oferir un botó cap a Configuració del Sistema.
@MainActor
@Observable
final class RadarStore {
    ...
    init(refreshInterval: TimeInterval = 6 * 60) {
        ...
        // `currentSystemAppearance()`: el seu `init` aplica el tema
        // PERSISTIT a `NSApp.appearance`, i `effectiveAppearance` només
        // reflecteix aquest tema un cop s'ha aplicat. Sense aquest ordre,
        // un tema persistit "Fosc" amb el sistema real en clar compondria
        // els 10 primers frames en clar (flaix visible en arrencar) fins
        // que el popover s'obrís i s'autocorregís.
        _ = AppearancePreference.shared
        // Llegida abans del primer `refresh()`, no esperant que la vista
        // cridi `setAppearance` en aparèixer: sense això, amb el sistema en
        // fosc, els 10 primers frames es compondrien en clar i tot seguit es
        // tornarien a compondre en fosc - doble feina i parpelleig visible.
        ...
        // Forat de privacitat tancat aquí: `location` és inert (vegeu
        // `LocationProvider`) i `RainNotifier.requestAuthorization()` no es
        // crida mai des d'aquest `init`. L'únic camí cap a qualsevol permís
        // és `enableAlerts()`, enganxat aquí sota - amb els avisos
        // desactivats (per defecte) no hi ha cap diàleg de permís durant
        // tot aquest `init`.
        AlertPreferences.shared.onEnabledChange = { [weak self] enabled in
            // `[weak self]`: `AlertPreferences.shared` viu tot el procés, i
            // una referència forta el convertiria en immortal.
            Task { @MainActor in
        ...
        // `enableAlerts()` es quedaria amb `coordinate` encara `nil` i
        // l'usuari no veuria severitat ni municipi fins al pròxim cicle de
        // refresc (fins a 6 min).
        ...
        // l'instant amb el frame que ja hi ha, no esperar el pròxim cicle.
        ...
        // l'instant, amb el popover ja obert - vegeu
        // `AppearancePreference.onModeChange` sobre per què no n'hi ha prou
        // amb `MenuBarContentView.onChange(of: colorScheme)`. "Sistema" es
        // resol contra l'aparença real del sistema en aquest instant, ja
        // que `FrameAppearance` només té `.light`/`.dark`.
        ...
        // reengegar la ubicació: amb el permís ja concedit no ensenya cap
        // diàleg, però sí cal cridar-lo perquè el `CLLocationManager`,
        // novament creat en aquest procés, torni a emetre actualitzacions.
        // No es torna a demanar autorització de notificacions: el sistema
        // recorda la decisió entre llançaments.
        if AlertPreferences.shared.alertsEnabled {
            location.requestPermissionAndStart()
        }

        // No cal cridar `startTimer()` a banda: `refresh()` ja en programa
        // un al final, incloent-hi aquesta primera crida.
        Task { await refresh() }
    }
    ...
    /// `refresh()` (encertat o no). Així dos refrescos disparats pel TIMER
    /// mai cauen més a prop l'un de l'altre que `refreshInterval` - amb un
    /// `Timer` repetitiu de veritat, un refresc disparat per
    /// `refreshIfNeededOnAppear` (obrir el popover) i el pròxim tic periòdic
    /// podien caure a pocs segons l'un de l'altre, duplicant la crida de
    /// xarxa. No evita que l'usuari mateix en dispari dos de propers obrint
    /// el popover repetidament (`openRefreshMinInterval` s'encarrega d'això
    /// per separat) - això és volgut, l'usuari vol dades fresques.
    func startTimer() {
        timer?.invalidate()
    ...
    /// Únic punt que demana permisos: ubicació primer, que és
    ...
    /// `enqueueAppearanceUpdate`. `RadarStore` és `@MainActor` però `await`
    /// és un punt de reentrada: dues crides gairebé simultànies (p.ex. el
    /// timer de 6 min disparant just quan l'usuari canvia de tema) podrien
    /// entrellaçar-se dins `RadarAnimator.build`/`recolor` si es cridessin
    /// directament, deixant `frames`/`currentIndex` a mitges. Llegir i
    /// assignar `pendingRebuild` sense cap `await` entremig ho evita: mai hi
    ...
    /// A diferència d'`enqueueRebuild` (un timestamp nou sempre val la pena
    /// compondre'l), aquí es comprova quan toca el torn a la cua si
    /// `newAppearance` encara és el valor vigent - si l'usuari ha triat un
    /// altre tema mentrestant, aquesta crida ja ha quedat obsoleta i la que
    /// ve darrere (amb el valor de veritat) farà la feina real. Sense això,
    /// triar tres temes de pressa recompondria els deu frames tres vegades
    /// seguides, encara que només l'última importi.
